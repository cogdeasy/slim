 ...
 *
 * These are defined locally rather than via `dmv.color.createColormap` because
 * the DMV colormaps are built for DICOM palette colour lookup tables (they are
 * consumed as `PaletteColorLookupTable` by the parametric-map path) whereas the
 * heatmap needs a plain RGB ramp it can write into an `ImageData` buffer.
 */
...
/**
 * Get a 256-entry RGB lookup table (flat, length 768) for a colormap.
 */
...
export function getColormapGradientCss(name: HeatmapColormapName): string {
  const ramp = RAMPS[name]
  const stops = ramp.map((rgb, index) => {
    const percentage = (index / (ramp.length - 1)) * 100
